...
impl Vector<f32> for f32x4 {
    ...
    fn select(mask: Self::Mask, if_true: Self, if_false: Self) -> Self {
        f32x4_bitselect(if_true, if_false, mask)
    }
...
impl Vector3<f32> for f32x4 {
    ...
    fn into_array(self) -> [f32; 3] {
        let mut out: MaybeUninit<f32x4> = MaybeUninit::uninit();
        unsafe {
            f32x4_store(out.as_mut_ptr(), self);
            *(&out.assume_init() as *const f32x4 as *const [f32; 3])
        }
    }
    ...
    fn into_tuple(self) -> (f32, f32, f32) {
        ...
        unsafe {
            f32x4_store(out.as_mut_ptr(), self);
            *(&out.assume_init() as *const f32x4 as *const (f32, f32, f32))
        }
    }
...
impl Vector4<f32> for f32x4 {
    ...
    fn into_array(self) -> [f32; 4] {
        ...
        unsafe {
            f32x4_store(out.as_mut_ptr(), self);
            *(&out.assume_init() as *const f32x4 as *const [f32; 4])
        }
    }
    ...
    fn into_tuple(self) -> (f32, f32, f32, f32) {
        ...
        unsafe {
            f32x4_store(out.as_mut_ptr(), self);
            *(&out.assume_init() as *const f32x4 as *const (f32, f32, f32, f32))
        }
    }
...
impl SignedVector3<f32> for f32x4 {
    ...
    #[inline]
    fn signum(self) -> Self {
        const NEG_ONE: f32x4 = const_f32x4!([-1.0; 4]);
...
impl From<f32x4> for XYZW<f32> {
    ...
    fn from(v: f32x4) -> XYZW<f32> {
        let mut out: MaybeUninit<f32x4> = MaybeUninit::uninit();
        unsafe {
            f32x4_store(out.as_mut_ptr(), v);
            *(&out.assume_init() as *const f32x4 as *const XYZW<f32>)
        }
    }
}
...
impl From<f32x4> for XYZ<f32> {
    ...
    fn from(v: f32x4) -> XYZ<f32> {
        ...
        unsafe {
            f32x4_store(out.as_mut_ptr(), v);
            *(&out.assume_init() as *const f32x4 as *const XYZ<f32>)
        }
    }
}
...
impl From<f32x4> for XY<f32> {
    ...
    fn from(v: f32x4) -> XY<f32> {
        ...
        unsafe {
            f32x4_store(out.as_mut_ptr(), v);
            *(&out.assume_init() as *const f32x4 as *const XY<f32>)
        }
    }
}
